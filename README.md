...
* A user can choose between V1 and V2 version of ParksApi.
* In API version v1, only basic CRUD functionality is deployed 
* In API version v2, uses pagination to display 2 db entries per page

### Set Up and Run Project

1. Clone this repo.
2. Open the terminal and navigate to this project's production directory called "ParksApi".
3. Within the production directory "ParksApi", create two new files: `appsettings.json` and `appsettings.Development.json`.
...
7. Within the production directory "ParksLookUpAPI", run `dotnet watch run --launch-profile "ParksLApi-Production"` in the command line to start the project in production mode with a watcher. 
